...
const RegisterPage = () => {
  ...
  const universityData: Array<SelectItem> = universitiesList!.map(
    (university) => {
      return {
  ...
  const userTypeIcons = [
    ...
    <IconClipboardText size="1rem" />,
    <IconBrandGoogle size="1rem" />,
  ];
  const allUserTypes = [
    UserType.Lead,
    UserType.CoreTeamMember,
    UserType.Facilitator,
    UserType.Googler,
  ];
  const userTypeData: Array<SelectItem> = allUserTypes.map(
    (userType, index) => {
      return {

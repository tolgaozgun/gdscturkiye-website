...
const ChangeUserProfileForm = ({
  ...
  const form = useForm({
    ...
    validate: {
      ...
      surname: (value) =>
        value && value.length > 0 ? null : "Surname is required.",
      biography: (value) =>
        value && value.length > 0 ? null : "Biography is required.",
    },
  });
  ...
  const { mutateAsync: submitProfile } = useMutation({
    ...
    mutationFn: (
      updateUserProfileByUserRequest: UpdateUserProfileByUserRequest
    ) => updateUserProfileByUser(axiosSecure, updateUserProfileByUserRequest),
    onError: (error: any) =>
      notifications.show({
  ...
  const handleSubmit = async () => {
    ...
    const userDetails: UpdateUserProfileByUser = {
      ...
      profileImage: form.isTouched("profileImage")
        ? form.values.profileImage
        : undefined,
      phoneNumber: form.isTouched("phoneNumber")
        ? form.values.phoneNumber
        : undefined,
      biography: form.isTouched("biography")
        ? form.values.biography
        : undefined,
      interests: form.isTouched("interests")
    ...
    if (isErrorResponse(res)) {
      notifications.show({
        message:
          res?.msg || "Something went wrong. Couldn't update the profile",
        color: "red",
      });
    } else {
      notifications.show({
    ...
    <Flex direction={"column"} gap={"xl"} p={padding} mt={mt}>
      <form>
        <Flex direction={"column"} gap={"md"}>
          ...
          >
            <CustomTextEditor
              content={user && user.biography ? user.biography : ""}
          ...
          <MultiSelect
            ...
            defaultValue={
              user?.interests
                ? user.interests.map((interest) => String(interest.topicId))
                : []
            }
            searchable
            withAsterisk

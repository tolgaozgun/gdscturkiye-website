...
const CustomCountrySelectItem = forwardRef<
  HTMLDivElement,
...
const AddCampaignForm = ({ padding, mt }: AddCampaignFormProps) => {
  ...
  const form = useForm({
    ...
    validate: {
      ...
      latitude: (value) =>
        value > 0 ? null : "Latitude must be greater than 0",
      longitude: (value) =>
        value > 0 ? null : "Longitude must be greater than 0",
    },
  });
  const {
    data: allCountries,
  ...
  const countryData: Array<SelectItem> = countryList!.map((country) => {
    return {
      name: country.name,

import { Affix, Flex, Input, rem } from "@mantine/core";
...
const AddQuestionForm = ({ padding, mt }: AddQuestionFormProps) => {
  ...
  const form = useForm({
    ...
    validate: {
      question: (value) =>
        value.trim().length > 0 ? null : "Question is required",
    },
  });

  const { mutateAsync: addQuestion } = useMutation({
    mutationKey: ["addQuestion"],
    mutationFn: (addQuestion: AskQuestion) =>
      askQuestion(axiosSecure, addQuestion),
    onError: (error: any) =>
      notifications.show({

  ...
  return (
    ...
    >
      <Card.Section>
        <Grid p={padding}>
          <Grid.Col span={4}>
            <Image src={coreTeamMember.user.profileImage} alt="No way!" />
          </Grid.Col>
          <Grid.Col span={8}>
            ...
            <Text color="black" weight={300} size="sm" mt={1}>
              {coreTeamMember.university.city.name},{" "}
              {coreTeamMember.university.country.name}
            </Text>
          </Grid.Col>
        </Grid>
      </Card.Section>
      {coreTeamMember.user.interests?.map((topic) => {
        return <Badge>{topic.name}</Badge>;
      })}
    </Card>
  );
};
